...
public class AutoDetector {
    private static final Logger LOGGER = LoggerFactory.getLogger(org.apdplat.word.util.AutoDetector.class);
    //已经被监控的文件
    ...
    private static final Map<DirectoryWatcher, org.apdplat.word.util.ResourceLoader> RESOURCE_LOADERS = new HashMap<>();
     ...
     */
    public static void loadAndWatch(org.apdplat.word.util.ResourceLoader resourceLoader, String resourcePaths) {
     ...
     * @throws IOException
     */
    private static List<String> loadClasspathResource(String resource, org.apdplat.word.util.ResourceLoader resourceLoader, String resourcePaths) throws IOException{
        List<String> result = new ArrayList<>();
        LOGGER.info("类路径资源："+resource);
        Enumeration<URL> ps = org.apdplat.word.util.AutoDetector.class.getClassLoader().getResources(resource);
     ...
     * @return 资源内容
     */
    private static List<String> loadHttpResource(String resource, org.apdplat.word.util.ResourceLoader resourceLoader) throws MalformedURLException, IOException {
        List<String> result = new ArrayList<>();
    ...
    private static void watchHttp(String resource, final org.apdplat.word.util.ResourceLoader resourceLoader){
    ...
    private static final class HttpResourceChangeRedisListener extends JedisPubSub {
        private org.apdplat.word.util.ResourceLoader resourceLoader;
        public HttpResourceChangeRedisListener(org.apdplat.word.util.ResourceLoader resourceLoader){
     ...
     * @throws IOException
     */
    private static List<String> loadNoneClasspathResource(String resource, org.apdplat.word.util.ResourceLoader resourceLoader, String resourcePaths) throws IOException {
        List<String> result = new ArrayList<>();
     ...
     * @return 目录所有资源内容
     */
    private static List<String> loadAndWatchDir(Path path, org.apdplat.word.util.ResourceLoader resourceLoader, String resourcePaths) {
        ...
        DirectoryWatcher.WatcherCallback watcherCallback = new DirectoryWatcher.WatcherCallback(){
            ...
            public void execute(WatchEvent.Kind<?> kind, String path) {
                ...
                if(System.currentTimeMillis() - lastExecute > 1000){
                    ...
                    synchronized(org.apdplat.word.util.AutoDetector.class){
                        DirectoryWatcher dw = WATCHER_CALLBACKS.get(this);
                        String paths = RESOURCES.get(dw);
                        org.apdplat.word.util.ResourceLoader loader = RESOURCE_LOADERS.get(dw);
                        LOGGER.info("重新加载数据");
    ...
    private static List<String> load(String path) {
        ...
        try{
            ...
            if(path.startsWith("classpath:")){
                in = org.apdplat.word.util.AutoDetector.class.getClassLoader().getResourceAsStream(path.replace("classpath:", ""));
            }else{
     ...
     * @param file 文件
     */
    private static void watchFile(final File file, org.apdplat.word.util.ResourceLoader resourceLoader, String resourcePaths) {
    ...
    public static void main(String[] args){
        loadAndWatch(new ResourceLoader(){
