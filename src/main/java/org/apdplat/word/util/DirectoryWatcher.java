...
public class DirectoryWatcher {
    private static final Logger LOGGER = LoggerFactory.getLogger(org.apdplat.word.util.DirectoryWatcher.class);
    ...
    public static org.apdplat.word.util.DirectoryWatcher getDirectoryWatcher(final WatcherCallback watcherCallback, WatchEvent.Kind<?>... events){
        if("true".equals(WordConfTools.get("auto.detect", "true"))){
            return new org.apdplat.word.util.DirectoryWatcher(watcherCallback, events);
        }
        LOGGER.warn("注意：未启用自动检测功能！如需启用，请在word.local.conf文件中指定配置项auto.detect=true");
        return new org.apdplat.word.util.DirectoryWatcher(){
            @Override
            public void close() {}
            @Override
    ...
    private DirectoryWatcher(final WatcherCallback watcherCallback, WatchEvent.Kind<?>... events){
        try {
            ...
            synchronized(org.apdplat.word.util.DirectoryWatcher.class){
    ...
    public static void main(String[] args) {
        org.apdplat.word.util.DirectoryWatcher dictionaryWatcher = new org.apdplat.word.util.DirectoryWatcher(new WatcherCallback(){
