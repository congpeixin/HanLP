...
public class Trigram {
    private static final Logger LOGGER = LoggerFactory.getLogger(org.apdplat.word.corpus.Trigram.class);
    ...
    public static float trigram(List<Word> words){
        if(words.size() > 2){
            ...
            for(int i=0; i<words.size()-2; i++){
                score += getScore(words.get(i).getText(), words.get(i+1).getText(), words.get(i+2).getText());
            }
            return score;
        }
        return 0;
